...
- When rendered testing is authorized, confirm the chosen wedge snaps bright and shows `DRIFT ASSIST` after a roughly 0.18-second hold; then stop chasing it while the auto-drift carries into a side skid, and throw the cursor far forward to accelerate out. If the confirmation still cannot be reached, tune the entry threshold or arm time rather than making the target visually larger again.
