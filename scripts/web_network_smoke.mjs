...
async function run() {
  ...
  const capture = (text, source, level = "info") => {
    consoleMessages.push({ source, level, text, time_msec: Date.now() });
  ...
  socket.addEventListener("message", event => {
    ...
    if (message.method === "Runtime.consoleAPICalled") {
      ...
      capture(text, "console", message.params.type);
    } else if (message.method === "Runtime.exceptionThrown") {
      errors.push({ source: "exception",
  ...
  const twoPlayerLines = () => consoleMessages.filter(message =>
    /\[car-fight:client\] CLIENT_TICK .*players=2 world=[^|]+\|/.test(message.text));
  const drive = async () => {
  ...
  await drive();
  await waitForValue(() => twoPlayerLines()[0], 15000, "first shared-world sample");
  ...
  const steady = fpsValues.slice(-5);
  ...
  const worldSnapshots = twoPlayerLines().map(message =>
  ...
  const report = {
    ...
    first_shared_world_samples: twoPlayerLines().length,
  ...
  report.pass = firstId > 1 && secondId > 1 && firstId !== secondId
    && report.first_shared_world_samples >= 2 && report.replacement_samples >= 5
    ...
    && report.steady_fps.minimum >= 30 && report.steady_fps.average >= 45
  ...
  console.log(`peers=${firstId}->${secondId} shared=${report.first_shared_world_samples}`
    ...
    + ` buffer_final=${report.webrtc_buffered_bytes.final} errors=${errors.length}`);
  socket.close();
