...
# circuit car, one shaped relay, and two monitored native clients.
set -euo pipefail
unsetopt BG_NICE
...
run_root="$project_root/.crash-runs/shaped-local-two-$profile-$run_stamp"
server_pid=""
proxy_pid=""
...
echo "shaped local two-client play: profile=$profile"
...
CAR_FIGHT_G2_STACK=1 CAR_FIGHT_HOST=127.0.0.1 CAR_FIGHT_PORT="$proxy_port" \
