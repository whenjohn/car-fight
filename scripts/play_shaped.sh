...
run_root="$project_root/.crash-runs/shaped-$profile-$run_stamp"
proxy_pid=""
...
echo "shaped native play: profile=$profile server=$server_host:$server_port"
...
CAR_FIGHT_SESSION_LABEL="network-$profile" \
	...
	--name "${CAR_FIGHT_NAME:-shaped-driver}"
