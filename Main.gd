...
func _ready() -> void:
	_parse_args()
	# This checkout is commonly run beside other Car Fight worktrees. Keep the
	# branch purpose visible in the native title bar whenever it owns a client
	# window, including a plain one-client launch with no explicit --client.
	if _role == "client" and DisplayServer.get_name() != "headless":
		DisplayServer.window_set_title("Car Fight — Area Weapon")
...
func _process(_delta: float) -> void:
	...
	if _status_label != null:
		...
		_status_label.text = "CAR FIGHT  |  %s  |  peer %d  |  %.1f u/s\n%s\n%s" % [
			...
			else "Mouse: drive  |  3: area weapon  |  Q: shield  |  R: cloak  |  Shift: vacuum  |  Space: burst  |  Tab: reverse  |  E: editor  |  C: cones"]
