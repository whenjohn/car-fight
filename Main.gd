...
func _process(_delta: float) -> void:
	...
	if _status_label != null:
		...
		if not _combat_editor_active and local != null and bool(local.get("is_cloaked")):
			...
			mode = "DRIVE + SHIELDED"
		elif not _combat_editor_active and local != null and float(local.call("det_radius")) > 0.0:
			mode = "DRIVE + DET ACTIVE"
		_status_label.text = "CAR FIGHT  |  %s  |  peer %d  |  %.1f u/s\n%s\n%s" % [
			...
			else "Mouse: drive  |  Cmd: det  |  Q: shield  |  R: cloak  |  Shift: vacuum  |  Space: burst  |  Tab: reverse  |  E: editor  |  C: cones"]
	if _fps_label != null:
