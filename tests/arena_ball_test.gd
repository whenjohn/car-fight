...
const BALL_SCRIPT_PATH := "res://world/arena_ball.gd"
...
func _init() -> void:
	...
	_check(ball_script != null, "arena ball script loads")
	if ball_script != null:
		_check(float(ball_script.RADIUS) >= 1.0, "ball is large enough to read at the isometric scale")
		...
		_check(spawn.distance_to(Vector3(-3.0, 0.0, 0.0)) > float(ball_script.RADIUS) + 1.55,
			"ball does not overlap the first car spawn")

	if _failures.is_empty():
		print("ARENA_BALL_TEST PASS")
		quit(0)
	else:
